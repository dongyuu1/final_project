...
from models.octree import Octree, merge_octrees
import builder
...
class SegSolver(Solver):
  ...
  def process_batch(self, batch, flags):
    def points2octree(points):
      octree = Octree(flags.depth, flags.full_depth)
  ...
  def model_forward(self, batch):
    ...
    logit, _, _ = self.model(data, octree, octree.depth, query_pts)
...
def main():
  flags = parse_args()
  model = get_segmentation_model(flags.MODEL).cuda()
  model.load_state_dict(torch.load("./logs/scannet/octformer_scannet/checkpoints/00540.model.pth"))
  dataset, collate_fn = get_segmentation_dataset(flags.DATA.test)
  ...
  for i in tqdm(range(1), ncols=80, leave=False):
    ...
    with torch.no_grad():
      batch = process_batch(batch, flags.DATA.train)
      octree, points = batch['octree'], batch['points']
      ...
      logit, inter_attn_feat_list, cnn_inter_feat_list = model(data, octree, octree.depth, query_pts, visual=True)
      label_mask = points.labels > flags.LOSS.mask
      
      
      logit = logit[label_mask]
      labels = points.labels[label_mask]
      
      #sampling_mask = torch.bernoulli(torch.ones_like(labels) * 0.01) != 0.
      #print(sampling_mask)
      #logit = logit[sampling_mask]
      #labels = labels[sampling_mask]
      
      num_class = logit.shape[1]
      colors = torch.randint(low=0, high=256, size=(num_class, 3)) / 255
      max_logit_labels = torch.max(logit, dim=1)[1]
      
      point_wise_colors = colors[max_logit_labels].cpu().numpy()
      locations = points.points[label_mask].cpu().numpy()
      
      # print(locations)
      point_cloud = open3d.geometry.PointCloud()
      point_cloud.points = open3d.utility.Vector3dVector(locations)
      point_cloud.colors = open3d.utility.Vector3dVector(point_wise_colors)
      open3d.io.write_point_cloud("./segmentation.pcd", point_cloud)

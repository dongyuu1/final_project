...
class OctSphericalCNN(torch.nn.Module):
  def __init__(self, c_in, c_out, dim, num_heads, patch_size, drop_path, dilation, nempty, num_blocks):
    super().__init__()
    self.c_in = c_in
    self.c_out = c_out
    self.bias = torch.nn.Parameter(torch.randn(c_out)) if c_out is not None else None
    radiusEdges, thetaEdges, phiEdges = self.build_edges()
    self.radiusEdges = torch.tensor(radiusEdges, device="cuda")
    self.thetaEdges = torch.tensor(thetaEdges, device="cuda")
    self.phiEdges = torch.tensor(phiEdges, device="cuda")
    self.bn = torch.nn.BatchNorm1d(c_out) if c_out is not None else None
    self.relu = torch.nn.ReLU(inplace=True)
    self.conv1 = SphericalConv(c_in, c_in)
    self.conv2 = SphericalConv(c_in, c_in) if c_out is not None else None
    self.conv3 = SphericalConv(c_in, c_out) if c_out is not None else None
    self.layer = OctFormerStage(dim=dim, num_heads=num_heads, patch_size=patch_size,
        drop_path=drop_path,dilation=dilation, nempty=nempty, num_blocks=num_blocks,)



  def compute_weight_idx(self, octree, depth, visual=False):
    inter_feats = None
    input_points = octree.central_locations[depth]
    central_points = octree.central_locations[depth - 1][octree.pidx[depth]]
    if visual:
      inter_feats = (torch.stack([input_points, central_points], dim=0), octree.pidx[depth])
    #print(torch.max(input_points), torch.max(central_points), torch.min(input_points), torch.min(central_points))
    #print(torch.mean(torch.abs(input_points - central_points)), depth)
    scale = 2 ** (1 - depth)
    sph = self.xyz2sph(input_points - central_points, scale)
    return self.sph2convidx(sph, scale, depth), inter_feats


  def forward(self, features, octree, depth, out_channels, visual=False):
    
    weight_idx, cnn_inter_feats = self.compute_weight_idx(octree, depth, visual)
    
    n1 = weight_idx.shape[0]
    n2 = features.shape[0]
    expand_flag = False
    if n1 != n2:
      features = features[octree.nempty_mask(depth)>0]
      expand_flag = True
        
    #pad1, pad2, pad3 = torch.split(torch.zeros((3, octree.nnum[self.depth - 1], channels), device="cuda"), 1, dim=0)

    #print(depth, torch.sum(weight_idx==0), weight_idx.shape)
    #print(features.shape, expand_weight_idx.shape, depth, octree.nnum[depth], octree.nnum[depth-1])
    feat_conv1 = self.conv1(features, weight_idx)
    
    if expand_flag:
      data = torch.zeros((n2, features.shape[1]), device="cuda")
      data[octree.nempty_mask(depth)] = feat_conv1
      feat_conv1 = data
    
    inter_data, attn_matrices = self.layer(feat_conv1, octree, depth, visual)
    if self.conv2 is not None:
      feat_conv2 = self.conv2(features, weight_idx)
    
      if expand_flag:
        data = torch.zeros((n2, features.shape[1]), device="cuda")
        data[octree.nempty_mask(depth)] = feat_conv2
        feat_conv2 = data
        
      inter_data += feat_conv2
    
    feat_out = None
    if self.c_out is not None:
      conv_data = inter_data
      if expand_flag:
        conv_data = inter_data[octree.nempty_mask(depth)>0]
      data = self.conv3(conv_data, weight_idx)

      idx = octree.idx if depth == octree.depth + 1 else octree.pidx[depth]
      counts = octree.counts if depth == octree.depth + 1 else octree.pcounts[depth]

      feat_out = self.bn(self.merge_features(idx, counts, data) + self.bias)
      
      if expand_flag:
        data = torch.zeros((octree.nnum[depth-1], feat_out.shape[1]), device="cuda")
        data[octree.nempty_mask(depth-1)] = feat_out
        feat_out = data
   
    return feat_out, inter_data, attn_matrices, cnn_inter_feats
   

  def merge_features(self, idx, counts, features):
    features = scatter_add(features, idx, dim=0) / counts.unsqueeze(1)
    return features

  def xyz2sph(self, input, scale):
    x, y, z = input[:, 0] , input[:, 1], input[:, 2]
    #print("x", torch.mean(torch.min(x)))
    r = torch.sqrt(x ** 2 + y ** 2 + z ** 2) + 1e-12 * scale
  
    theta = torch.acos(z / r)
    phi = torch.atan(y / (x + 1e-12 * scale))
    return torch.stack([r, theta, phi], dim=1)

  def sph2convidx(self, input, scale, depth):
    
    r, theta, phi = input[:, 0], input[:, 1], input[:, 2]
    r_idx = torch.bucketize(r, self.radiusEdges[depth] * scale)
    
    #print(depth, torch.sum(r_idx==1), torch.sum(r_idx==2), torch.sum(r_idx==3))
    theta_idx = torch.bucketize(theta, self.thetaEdges)
    phi_idx = torch.bucketize(phi, self.phiEdges)
    idx = theta_idx + (phi_idx - 1) * 8 + (r_idx - 1) * 8 * 2
    idx[r <= (1e-2 * scale)] = 0
    max_t = torch.max(idx)
    idc = torch.argmax(idx, dim=0)
    del r, theta, phi, theta_idx, phi_idx, r_idx
    return idx
  ...
  def build_edges(self):
    ...
    for i in range(numTheta):
      thetaEdges.append(i * (PI / 4) - PI)
    
    for i in range(numPhi):
      phiEdges.append(i * (PI / 2) - PI / 2)
    """
    for i in range(numRadius - 1):
      radiusEdges.append(float(i) / float(numRadius - 1))
    radiusEdges.append(1000)
    """
    return radiusEdges, thetaEdges, phiEdges


class SphericalConv(torch.nn.Module):
  def __init__(self, c_in, c_out):
    super().__init__()
    self.c_in = c_in
    self.c_out = c_out
    self.weights = torch.nn.Parameter(torch.randn(49, c_in, c_out))
    self.relu = torch.nn.ReLU(inplace=True)
    self.bn = torch.nn.BatchNorm1d(c_out) 

  def forward(self, features, weight_idx):

    N, _ = features.shape
    out_features = torch.zeros((N, self.c_out), device="cuda")
    for i in range(49):
      part_feat = features[weight_idx==i]
      if part_feat.nelement() > 0:
        conv_part_feat = part_feat @ self.weights[i]
        out_features[weight_idx==i] = conv_part_feat
        
    out_features = self.relu(self.bn(out_features))
    #feat_conv = self.relu(self.bn(torch.sum(features.unsqueeze(dim=2) * self.weights[weight_idx], dim=1)))
    return out_features
...
class OctreeAttention(torch.nn.Module):
  ...
  def forward(self, data: torch.Tensor, octree: OctreeT, depth: int, visual=False):
    H = self.num_heads
    K = self.patch_size
    C = self.dim
    D = self.dilation
...
class OctFormerBlock(torch.nn.Module):
  ...
  def forward(self, data: torch.Tensor, octree: OctreeT, depth: int, visual=False):
    ...
    attn, inter_feat = self.attention(self.norm1(data), octree, depth, visual)
  ...
  def forward(self, data: torch.Tensor, octree: OctreeT, depth: int, visual=False):
    ...
    for i in range(self.num_blocks):
      if self.use_checkpoint and self.training:
        data, inter_feat = checkpoint(self.blocks[i], data, octree, depth, visual)
      else:
        data, inter_feat = self.blocks[i](data, octree, depth, visual)
      inter_feat_list.append(inter_feat)
    return data, inter_feat_list
...
class OctFormer(torch.nn.Module):
  ...
  def __init__(self, in_channels: int,
    ...
    #self.patch_embed = SphericalPatchEmbed(in_channels, channels[0])
    self.patch_embed = PatchEmbed(in_channels, channels[2], stem_down, nempty)
    self.integrated_layers = torch.nn.ModuleList([OctSphericalCNN(c_in=channels[i+2],
      c_out=channels[i+3] if i < self.num_stages-1 else None,
      dim=channels[i+2], num_heads=num_heads[i], patch_size=patch_size,
      drop_path=drop_ratio[sum(num_blocks[:i]):sum(num_blocks[:i+1])], dilation=dilation, nempty=nempty,
      num_blocks=num_blocks[i],) for i in range(self.num_stages)])
    """
    ...
    self.spherical_cnns = torch.nn.ModuleList([OctSphericalCNN(
      channels[i], channels[i+1]) for i in range(self.num_stages - 1)])
  ...
  def forward(self, data: torch.Tensor, octree: Octree, depth: int, visual=False):
    ...
    for i in range(self.num_stages):
      ...
      if self.num_blocks[i] != 0:
        data, inter_data, attn_features, cnn_features = self.integrated_layers[i](data, octree, depth_i,
                                            self.channels[i+1] if i < self.num_stages - 1 else self.channels[i], visual)
        features[depth_i] = inter_data
        attn_inter_feat_list.append(attn_features)
        cnn_inter_feat_list.append(cnn_features)

    return features, attn_inter_feat_list, cnn_inter_feat_list
